# -*- coding: utf-8 -*-
"""
# Estimación del reclutamiento criminal por estado
# mediante dinámica de sistemas
# Autores: Amador & Carbonell, LAB-CO (2026)
# Versionado: modelo por estado, 2019-2025
#
...
RUTA_EXCEL   = r"C:\Users\luisa\Desktop\Reclutamiento\Reclutamiento_Data frame_20260218.xlsx"  
ESTADOS_A_CORRER = []   # <<< MODIFICAR ("JALISCO", "SINALOA" o [] para TODOS)
...
RELIABILITY_W = dict(
    hom=3.5,   # homicidios (muy confiable)
    arm=2.5,   # armas (confiable)
    ext=1.3,
    nar=1.1,
    sec=0.9,
    sal=0.9,
...
VARS_DELITO = ["homicidios","secuestro","extorsion","salud","narcomenudeo",
               "financieros","hidrocarburos","trafico","armas"]
...
# Estas son las únicas cosas que los workers necesitan importar.
# No hacen I/O, no tocan globals mutables, son 100% serializables.
# ============================================================
...
def normalizar_estado(s):
    ...
    for a, b in [("A","A"),("E","E"),("I","I"),("O","O"),("U","U"),("U","U"),("N","N")]:
        pass  # no-op placeholder — reemplazos reales abajo
...
def calc_observables(sol, p, inputs):
    ...
    return {
        ...
        "trafico"      : np.clip(p["Tasa_reporte_trafico_personas"]     * p["tasa_trafico_personas_por_miembro"]     * A*dem,         0, None),
    }
...
def loss_rel_smooth(obs_pred, obs_real, denom_floor):
    pairs = [("hom","homicidios"),("sec","secuestro"),("ext","extorsion"),
             ...
             ("fin","financieros"),("hid","hidrocarburos"),("tra","trafico")]
    total = 0.0
    for wk, vk in pairs:
        ...
        fit_term = float(np.mean(rel_sq))  # promedio => comparable entre estados/ventanas
...
def draw_params(seed):
    ...
    return {
        ...
        "tasa_trafico_personas_por_miembro"     : r.uniform(0.00018, 0.00048),
        ...
        "Tasa_reporte_trafico_personas"         : r.uniform(0.60,  0.85),
    }
...
# ============================================================
# PUNTO DE ENTRADA
# TODO lo que produce I/O o llama al Pool va aquí.
# Los workers de Windows solo importan el módulo hasta la línea
# anterior — nunca ejecutan el bloque main.
# ============================================================
...
if __name__ == "__main__":
    ...
    sheets_vars = [
        ...
        ("HIDROCARBUROS", "hidrocarburos"), ("TRAFICO", "trafico"),
        ("ARMAS", "armas"),
    ]
    ...
    for estado in estados_finales:
        ...
        with Pool(processes=N_WORKERS) as pool:
            for i, res in enumerate(pool.imap_unordered(run_one_scenario, args_list), 1):
                ...
                if res is not None:
                    ...
                    A_dic_2025 = float(res["A_series"][-1])   # último mes (Dic 2025 si llega hasta ahí)
        ...
        A_all = np.vstack([r["A_series"] for r in results_raw])  # shape: (n_scen, n_meses)
        best  = min(results_raw, key=lambda r: r["loss"])
    ...
    # Para 1 solo estado (el primero en estados_finales)
    # ============================================================
    ...
    #   - n_meses
    #   - estado (string)
    # Si corriste múltiples estados, pon esto dentro del loop "for estado in estados_finales:".
    ...
    if results_raw:
        ...
        # Eje X en meses 1..12 o 1..n_meses
        x = np.arange(1, n_meses + 1)
    
        # Etiquetas anuales tipo Ene\n2019 (cada 12 meses)
        ...
        #    Para que no se vea demasiado oscuro, usamos alpha bajo
